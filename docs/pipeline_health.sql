...
CREATE OR REPLACE VIEW tennis_raw.vw_data_quality_summary AS ADD
...
SELECT 
    ...
    CASE 
        ...
        WHEN SAFE_DIVIDE(LEAST(winner_count, loser_count), GREATEST(winner_count, loser_count)) < 0.9 THEN 'WARN'
        ELSE 'PASS'
    END AS status,
    CONCAT(
